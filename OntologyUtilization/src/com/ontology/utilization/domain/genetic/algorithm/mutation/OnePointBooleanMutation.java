...
public class OnePointBooleanMutation extends BooleanMutation {

	private BooleanEstimate estimate;

	public OnePointBooleanMutation(int probabilityOfMutation,
		...
		for (BooleanChromosome ch : chromosomes) {
			...
			for (int cecha = 0; cecha < 9; cecha++) {
				...
				if (random < probabilityOfMutation) {
					...
					Integer mutationPoint = rGen.nextInt(10);

					mutationChromosome
							.getDescription()
							.get(cecha)
							.set(mutationPoint,
									!(mutationChromosome.getDescription().get(
											cecha).get(mutationPoint)));
